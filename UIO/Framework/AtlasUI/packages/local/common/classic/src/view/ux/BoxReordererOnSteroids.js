/**
 * Base class for ProSvc.ux.TabReordererOnSteroids.
 */
Ext.define('Atlas.common.view.ux.BoxReordererOnSteroids', {
    requires: [
        'Ext.dd.DD'
    ],
    ...
    onBoxReady: function() {
        ...
        Ext.apply(dd, {
            ...
            doSwap: me.doSwap,
            doSwapContainer: me.doSwapContainer, // #OnSteroids: Main method for allowing swapping tabs between tab panels
    ...
    /**
     * @private
     * Swap the container context.
     * @param {String} newContainerId Id of the new container ({@link Ext.tab.Bar}) we're entering during the drag action
     */
    doSwapContainer: function (newContainerId, e) {
        console.log('doSwapContainer', newContainerId);

        var me = this,
            newContainer = Ext.getCmp(newContainerId),
            items = me.container.items,
            newItem,
            orig;
        ...
        newItem = newContainer.insert(0, orig);

        me.startIndex = me.curIndex = 0;
        me.deltaSetXY = null;

        if (newContainer == me.origContainer) {
            delete me.origContainer;
        } else {
            // only set origContainer if it's not set already
            // when having multiple tab panels, we may be swapping containers multiple times during a drag action,
            // but we don't want to lose the original container where the dragging started
            if (!me.origContainer) {
                me.origContainer = me.container;
            }
        }

        // updated layout of previous container e.g. to reset possible tab space that was left when passing the
        // container during drag action (e.g. drag tab to another tab bar to first position and then drag it back
        // without dropping in between)
        me.container.updateLayout();

        // this here is doing most of the magic. we assign the container context (tab bar) to the new container that
        // we enter during the drag action
    ...
    onDrag: function(e) {
        ...
        // #OnSteroids: Here we check if mouse enters into another tab bar, if so, we call the doSwapContainer method
        tabBar = e.getTarget('.x-tab-bar');
        if (tabBar && tabBar.id != me.container.id) {
            me.doSwapContainer(tabBar.id, e);
        }
