...
Ext.define('Atlas.macprice.view.MacSetupController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.MacSetupController',
    ...
    OnUserListLoad: function(store, records, success) {
        ...
        var hasAccess = store.data.items.length > 0;

        if (store.storeId == 'MACExecutive') {
            vm.set('MACExecutive', hasAccess);
        }
        else {
            vm.set('MACAnalyst', hasAccess);
        }
