 ...
 */
Ext.define('Atlas.benefitplan.view.configuration.PopGroupPlanBenefitPackageSummaryController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.popgroupplanbenefitpackagesummaryController',
    listen : {
    ...
    onFilterPackages:function () {
        ...
        if(this.lookupReference('isActiveFlag').checked) {
    ...
    onSelectionChange: function (row, selected) {
        ...
        {
            vm.set('PBPSK', selected[0].data.PBPSK);
            vm.set('popGrpPBPChange',!selected[0].data.isLocked);
            vm.set('popGrpChangeDetails',selected);
            me.getView().down('[itemId="populationGroupPlanBenefitPackageSummaryGrid"]').getPlugin('rowEditing').disable();
        }
    },
