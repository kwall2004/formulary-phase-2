...
namespace AtlasWebApi.Controllers.V1.DrugDetail
{
    ...
    /// </summary>
    public class MissingNdcMedispanController : ApiController
    {
        ...
        public MissingNdcMedispanController(IReferenceRepositoryFactory refFactory, IExceptionMessageGenerator exceptionResponseGenerator, IFormularyRepositoryFactory formularyFactory)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public IHttpActionResult getAllCreatedMissingNDCs ()
        {
            try
            {
                using (var repo = _refFactory.FDBDrugList())
                {
                    using (var ndcTypeRepo = _refFactory.NDCType())
                    {
                        var results = new List<FDBDrugList>();
                        var ndcType = ndcTypeRepo.GetNdcType("Medispan");
                        ...
                        var returnedResults = new QueryResult<FDBDrugList> { Rows = results, Count = results.Count() };
        ...
        /// <returns></returns>
        [HttpPost]
        public IHttpActionResult saveMissingNDC (FDBDrugList missingNDC)
        {
            try
            {
        ...
        /// <returns></returns>
        [HttpPut]
        public IHttpActionResult updatedMissingNDC(FDBDrugList missingNDC)
        {
            try
            {
                return AddOrUpdatedMissingNDC(missingNDC);
        ...
        private IHttpActionResult AddOrUpdatedMissingNDC(FDBDrugList missingNDC)
        {
            using (var repo = _refFactory.FDBDrugList())
            {
                using (var ndcTypeRepo = _refFactory.NDCType())
                {
                    ...
                    return Ok(missingNDC.DrugListSK); 
                }
            }
        }
    }
