...
{
    public class MissingNDCFdbSearchController : ApiController
    {
        ...
        public MissingNDCFdbSearchController(IExceptionMessageGenerator exceptionResponseGenerator, IFormularyRepositoryFactory FormularyRepoFactory)
        {
        ...
        public IHttpActionResult Search(string queryString)
        {
            try
            {
                var result = new QueryResult<spMissingNDCFDB_Search_Result>();

                using (var repo = _FormularyRepoFactory.DrugSearch())
                {
                    var queryResult = repo.MissingNDCFdbSearch(queryString);
